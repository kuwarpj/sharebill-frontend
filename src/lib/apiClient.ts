...
export async function apiClient<TResponse, TBody = unknown>(
  ...
  const { method, endpoint, body, params, headers: customHeaders, isPublic = false } = options;
  ...
  if (!isPublic) {
    ...
    if (token) {
      ...
      console.warn(`API call to ${endpoint} without token for a non-public, non-GET request.`);
    }
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(response.statusText || `Request failed with status ${response.status} and non-JSON response`);
    }
